...
export const generateCars = (
	...
	const getLowMutationRange = () => {
		...
		return (
			MIN_LOW_MUTATION_RATE +
			Math.random() * (upperBound - MIN_LOW_MUTATION_RATE)
		);
	};
...
export const getRemainingCars = (cars: Array<RacingCar>) =>
	cars.filter((car) => !car.isDamaged());
