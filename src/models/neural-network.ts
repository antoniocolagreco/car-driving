...
export default class NeuralNetwork {
    ...
    // Serialization helpers to persist and restore instances
    toJSON() {
        return {
            id: this.id,
    ...
    static fromJSON(json: ReturnType<NeuralNetwork['toJSON']>): NeuralNetwork {
        // Derive neurons from layers when needed to ensure consistency
        const hasValidLayers = Array.isArray(json.layers) && json.layers.length > 0
        const derivedNeurons = hasValidLayers
            ? [json.layers[0].inputs.length, ...json.layers.map((l) => l.outputs.length)]
            : json.neurons

        const neuronCounts =
            Array.isArray(derivedNeurons) && derivedNeurons.length >= 2
                ? derivedNeurons
                : json.neurons && json.neurons.length >= 2
                  ? json.neurons
                  : [1, 1] // minimal safe network to avoid runtime errors

        const nn = new NeuralNetwork(...neuronCounts)
        nn.id = json.id
        nn.survivedRounds = json.survivedRounds
        nn.pointsRecord = json.pointsRecord

        // Copy weights/biases with bounds checks
        for (let i = 0; i < nn.layers.length; i++) {
            const src = json.layers[i]
            const dst = nn.layers[i]
            if (!src) continue
            dst.inputs = [...src.inputs]
            dst.outputs = [...src.outputs]
            dst.biases = [...src.biases]
            dst.weights = src.weights.map((row) => [...row])
        }
        return nn
    }
}
