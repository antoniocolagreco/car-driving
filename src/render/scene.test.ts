...
describe('drawScene traffic visibility', () => {
    ...
    it('passes the requested radar mode through to the sensor renderer', () => {
        drawScene(layer, state, { radarMode: 'zones' })

        expect(drawSensors).toHaveBeenCalledWith(layer.context, activeCar.sensorState, 'zones')
    })

    it('hides only the radar while continuing to draw traffic and racing cars', () => {
        drawScene(layer, state, { radarMode: 'off' })
...
describe('drawScene racing-car paint order', () => {
    ...
    it('draws random cars, winner, current winner and manual player in that exact order', () => {
        const random = racingCar(-120)
        const previousWinner = racingCar(-40)
        ...
        const orderedState: SimulationState = {
            ...state,
            cars: [player, winner, previousWinner, random],
            activeCar: winner,
            bestCar: winner,
            playerCar: player,
            winner: previousWinner.network,
            manualDriving: true,
        }
    ...
    it('draws traffic obstacles after the player and every other racing car', () => {
        ...
        const orderedState: SimulationState = {
            ...state,
            cars: [random, previousWinner, winner, player],
            activeCar: winner,
            bestCar: winner,
            playerCar: player,
            winner: previousWinner.network,
    ...
    it('uses no display offset when highlighted cars share the starting point', () => {
        ...
        expect(drawSensors).toHaveBeenCalledWith(layer.context, winner.sensorState, 'hull')
    })
